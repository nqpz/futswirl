...
let figure (trans: []point -> i32 -> i32 -> []point)
...
let figure_run_transform (array_dest: *[]point) (array: []point)
...
let figure2 (trans0: point -> point) (trans1: point -> point)
            (height: i32) (width: i32) (steps: i32):
            [height][width]argb.colour =
  let n = 2
  let trans (array: []point) (i: i32) (iinv: i32): []point =
    let array0 = copy array
    let array1 = figure_run_transform array0 array i iinv n trans0 0
    let array2 = figure_run_transform array1 array i iinv n trans1 1
    in array2
  in figure trans height width steps

let figure3 (trans0: point -> point) (trans1: point -> point)
            (trans2: point -> point)
            (height: i32) (width: i32) (steps: i32):
            [height][width]argb.colour =
  let n = 3
  let trans (array: []point) (i: i32) (iinv: i32): []point =
    ...
    let array1 = figure_run_transform array0 array i iinv n trans0 0
    let array2 = figure_run_transform array1 array i iinv n trans1 1
    let array3 = figure_run_transform array2 array i iinv n trans2 2
    in array3
  in figure trans height width steps

let figure4 (trans0: point -> point) (trans1: point -> point)
  ...
  let trans (array: []point) (i: i32) (iinv: i32): []point =
    ...
    let array2 = figure_run_transform array1 array i iinv n trans1 1
    let array3 = figure_run_transform array2 array i iinv n trans2 2
    let array4 = figure_run_transform array3 array i iinv n trans3 3
    in array4
  in figure trans height width steps

let foo = figure2
          (rotate 0.5 >-> translate (0, -0.08) >-> scale 0.99)
          (rotate (-0.5) >-> translate (0, 0.08) >-> scale 0.99)

let sierpinski = figure3
                 (translate ( 0,   -0.25) >-> scale 0.5)
                 (translate (-0.25, 0.25) >-> scale 0.5)
                 (translate ( 0.25, 0.25) >-> scale 0.5)
...
module lys: lys with text_content = text_content = {
  ...
  let init (_seed: i32) (h: i32) (w: i32): state =
    {height=h, width=w, iterations=12, time=0}
  ...
  let render (s: state) =
    swirl s.time s.height s.width s.iterations

  let text_format = "FPS: %d\nIterations: %d\nTime: %f"

  let text_content (fps: f32) (s: state): text_content =
    (t32 fps, s.iterations, s.time)
